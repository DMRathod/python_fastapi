...
def get_post(id: int, response: Response, userid: int = Depends(get_current_user)):
...
def create_post(post: UPosts, userid: int = Depends(get_current_user)):
    post = insert_data_in_uposts_table(post)
    return post

@router.put('/update/{id}', status_code=status.HTTP_200_OK, response_model=UPostOut)
def update_post(id: int, upost: UPosts, response: Response, userid: int = Depends(get_current_user)):
...
def delete_post(id: int, response: Response, userid: int = Depends(get_current_user)):
